...
\kdFF[#1]{#2}{#3}
\foreach \n in {1,2,3,4,6}{\draw[thin](\kref\pb\n)--(\kref\p\n);}
\foreach \n/\lbl in {1/J,3/K,4/{\overline{Q}},6/Q}{\draw[thin](\kref\pl\n)node[]{$\lbl$};}
\foreach \n in {2}{\draw[thin](\kref\pcd\n)--(\kref\pcm\n)--(\kref\pcu\n);}
\foreach \n in {4}{\draw[thin](\kref\pn\n)node[ocirc]{};}
}
%%%%%%%%%%%%%%%%%%%%%%
...
\draw[thin](\kshiftX,\kshiftY)coordinate(\kref\pin)--++(0,-\kpin)coordinate(\kref\pb\pin);
...
\pgfmathsetmacro{\kdimX}{2*\kulH+0*\kpsep}
\pgfmathsetmacro{\kdimY}{\kdimX}		%two spaces between 3 pins
\draw[thin](\kshift)coordinate(\kref\pin)--++(0,-\kpin)coordinate(\kref\pb\pin);
\draw(\kref\pb\pin)[thick]++(-\kdimY/2,0)coordinate(aa)--++(\kdimY,0)
coordinate(bb)--++(-\kdimY/2,-\kdimX)coordinate(cc)coordinate(\kref\pb\pout)--++(-\kdimY/2,\kdimX);
\draw[thin](\kref\pb\pout)--++(0,-\kpin)coordinate(\kref\pout);
\draw($(bb)!0.5!(cc)$)coordinate(\kref\pb\pu)+(0.07,0)coordinate(\kref\pn\pu)+(\kpin,0)coordinate(\kref\pu);
...
%%%%%%%%%%%%%%%%%%
\newcommand{\kBusORdown}[3][u0]{{	%double brace needed to keep variables \kxdim and \kydim local
\def\kref{#1}
\def\kshiftX{#2}
\def\kshiftY{#3}
\def\kin{in}
...
%======================
%%%%%%%%%%%%%%%%%%
\newcommand{\kBusANDdown}[3][u0]{{	%double brace needed to keep variables \kxdim and \kydim local
\def\kref{#1}
\def\kshiftX{#2}
\def\kshiftY{#3}
\def\kin{in}
...
\def\kxdim{1}
\def\kydim{1}
\draw[thick](\kshiftX,\kshiftY)coordinate(\kref\kout) to [out=0,in=-90] ++(0.5*\kydim,\kxdim)--++(-\kydim,0)coordinate[pos=0.5](\kref\kin) to [out=-90,in=180] (\kshiftX,\kshiftY);
}}
%======================
