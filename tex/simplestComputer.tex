...
\draw(0,0)  [thick] rectangle ++(\kdimX,\kdimY)node[pos=0.5,rectangle,inner sep=0pt,text width=1.5cm,align=center]{\text{\RL{ہدایت}} \text{\RL{گنت کار}}};
...
\draw(0,-\ksepY)  [thick] rectangle ++(\kdimX,\kdimY)node[pos=0.5,rectangle,inner sep=0pt,text width=2cm,align=center]{\text{\RL{برنامہ نویس}}\\ \text{\RL{اور}}\\ \text{\RL{دفتر پتہ}}};
...
\draw(0,-2*\ksepY)  [thick] rectangle ++(\kdimX,\kdimY)node[pos=0.5,rectangle,inner sep=0pt,text width=2cm,align=center]{$16\times 8$ \text{\RL{عارضی حافظہ}}};
...
\draw(0,-3*\ksepY)  [thick] rectangle ++(\kdimX,\kdimY)node[pos=0.5,rectangle,inner sep=0pt,text width=2cm,align=center]{ \text{\RL{دفتر ہدایت}}};
...
\draw(0,-4*\ksepY)  [thick] rectangle ++(\kdimX,\kdimY)node[pos=0.5,rectangle,inner sep=0pt,text width=3cm,align=center]{ \text{\RL{قابو  و ترتیب کار}}};
...
\draw(\ksepX,-0*\ksepY)  [thick] rectangle ++(\kdimX,\kdimY)node[pos=0.5,rectangle,inner sep=0pt,text width=1cm,align=center]{ \text{\RL{دفتر}} $A$};
...
\draw(\ksepX,-1*\ksepY)  [thick] rectangle ++(\kdimX,\kdimY)node[pos=0.5,rectangle,inner sep=0pt,text width=1cm,align=center]{ \text{\RL{جمع و منفی کار}}};
...
\draw(\ksepX,-2*\ksepY)  [thick] rectangle ++(\kdimX,\kdimY)node[pos=0.5,rectangle,inner sep=0pt,text width=1cm,align=center]{ \text{\RL{دفتر}} $B$};
...
\draw(\ksepX,-3*\ksepY)  [thick] rectangle ++(\kdimX,\kdimY)node[pos=0.5,rectangle,inner sep=0pt,text width=2cm,align=center]{ \text{\RL{خارجی دفتر}}};
...
\draw(\ksepX,-4*\ksepY)  [thick] rectangle ++(\kdimX,\kdimY)node[pos=0.5,rectangle,inner sep=0pt,text width=2cm,align=center]{ \text{\RL{ثنائی نمائشی تختی}}};
\end{tikzpicture}
...
\حصہ{ہدایات کی فہرست} 
کمپیوٹر کی با مقصد دوڑ سے قبل اس کے حافظہ  میں ہدایات قدم با قدم  بھرنا لازم ہے۔البتہ، ایسا کرنے سے پہلے  آپ کو یہ ہدایات  جاننی  ہو گی۔ان ہدایات سے مراد وہ اعمال ہیں جو یہ کمپیوٹر سرانجام دے سکتا ہے۔ اس کمپیوٹر کی ہدایات کی فہرست  پر اب غور کرتے ہیں۔ہدایت کا مجموعہ کمپیوٹر کی \اصطلاح{ مادری زبان }\فرہنگ{مادری زبان}\حاشیہب{assembly language}\فرہنگ{assembly language} کہلاتی ہے۔
...
اس کے بعد پتہ  اور مواد کے  سوئچ    قدم با قدم   درج ذیل رکھیں، جہاں \قول{ک} سے مراد  کھڑا   یعنی منقطع  سوئچ ہے جو \عددی{1} کو ظاہر کرتا ہے،  \قول{ب} سے مراد بیٹھا یا غیر منقطع (چالو) سوئچ ہے کو \عددی{0}   دیگا، اور \قول{x} سے مراد یہ کہ  سوئچ کسی بھی حالت (منقطع یا غیر منقطع)  میں ہو سکتا ہے۔
\begin{center}
\begin{tabular}{rr}
پتہ& مواد\\[1ex]
